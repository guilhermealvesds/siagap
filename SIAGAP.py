...
def cores():
	...
	def cores_cdp():
		...
		def consultar():
			...
			botaoFechar = Button(footer, text='FECHAR', command=combine_funcs(on_close_cdp_consultas, u.destroy))
			botaoFechar.pack(side=tk.RIGHT, padx = 5, pady = 5)
			botaoSelecionar = Button(footer, text='SELECIONAR', command=confirmar)
			botaoSelecionar.pack(side=tk.RIGHT, padx = 5, pady = 5)
		...
		if v_cores_cdp == 0:
			...
			botaoLimpar = Button(footer, image=v_cores_cdp_icon_img_4, relief=FLAT, command=limpar)
			botaoLimpar.grid(column=2, row=8)	
